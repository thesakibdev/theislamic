      ...
      for (const chapter of part.chapters) {
        for (const hadith of chapter.hadithList) {
          ...
          if (exists && exists._id.toString() !== this._id.toString()) {
            throw new Error(`Duplicate hadith found: bookName=${this.bookName}, partNumber=${part.partNumber}, chapterNumber=${chapter.chapterNumber}, hadithNumber=${hadith.hadithNumber}`);
          }
        }
      }
    }
